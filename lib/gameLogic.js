...
async function submitShot(sessionId, sessionHash, level, hit) {
  ...
  if (hit) {
    ...
    await db.run(
      "UPDATE game_sessions SET total_shots = total_shots + 1, total_misses = total_misses + 1, last_shot_at = datetime('now'), finished_at = datetime('now') WHERE id = ?",
      [sessionId]
    );

    const playerName = await db.queryOne('SELECT username FROM players WHERE id = ?', [session.player_id]);
    if (playerName) {
      await db.run("INSERT INTO activity_log (type, player_name, level, detail) VALUES ('miss', ?, ?, 'Game over')", [playerName.username, level]);
    }
